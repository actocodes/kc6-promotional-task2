...
TOT_RELEVANCE_THRESHOLD = 5.0
MAX_CHUNKS_RETURNED     = 6
...
def hierarchical_search(queries: list[str], top_k: int = 8) -> list[dict]:
    ...
    for l0_doc, l0_s in l0_scored[:2]:
...
async def tavily_fallback(query: str) -> list[dict]:
    ...
    try:
        ...
        logger.exception("Tavily fallback failed: %s", exc)
        return []
...
async def run_crag(query: str) -> str:
    ...
    if not scored:
        ...
        if web_docs:
            scored = [(d, 5.0) for d in web_docs]
